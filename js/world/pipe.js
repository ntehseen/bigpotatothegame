(function() {
  ...
  Pipe = Mario.Pipe = function(options) {
    ...
    // When statics already draw the well: hide pipe art and skip solid collision
    this.noRender = !!options.noRender;
  ...
  Pipe.prototype.checkPipe = function() {
    ...
    if (player.piping) return;

    // Hitbox-based. Must run AFTER collisions so feet sit on the pipe mouth.
    var hx = Math.floor(player.pos[0] + player.hitbox[0]);
    var hy = Math.floor(player.pos[1] + player.hitbox[1]);
    var hw = player.hitbox[2];
    var hh = player.hitbox[3];
    var feet = hy + hh;
    var cx = hx + hw / 2;
    var pipeTop = this.pos[1];
    var pipeLeft = this.pos[0];

    switch (this.direction) {
      case 'RIGHT':
        if (hx + hw >= pipeLeft - 4 && hx < pipeLeft + 8 &&
            hy >= pipeTop - 4 &&
            feet <= pipeTop + 36) {
        ...
        break;
      case 'LEFT':
        if (hx + hw > pipeLeft + 16 * this.length - 8 &&
            hx <= pipeLeft + 16 * this.length + 4 &&
            hy >= pipeTop - 4 &&
            feet <= pipeTop + 36) {
          player.pipe(this.direction, this.destination);
        }
        break;
      case 'UP':
        if (Math.abs(hy - (pipeTop + 16 * this.length)) <= 4 &&
            cx >= pipeLeft + 2 &&
            cx <= pipeLeft + 30) {
          player.pipe(this.direction, this.destination);
        }
  ...
  Pipe.prototype.checkCollisions = function() {
    ...
    if (this.solid && !player.piping) this.isCollideWith(player);
    // Enter only after feet have been resolved onto the pipe / well mouth
    if (!player.piping && this.destination) this.checkPipe();
  }
  ...
  Pipe.prototype.update = function(dt) {
    // Pipe entry is handled in checkCollisions (after feet resolve).
  }
